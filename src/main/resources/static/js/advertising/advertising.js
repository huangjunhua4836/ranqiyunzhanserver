...
layui.use('core', function(){
    ...
    function initTable(where){
        ...
        table.render({
            elem: '#table'
            ,url: '/platform/advertising/initTable' //数据接口
            ,where: where
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                core.openIframeDialog('添加','/platform/advertising/input?type=add',['500px', '900px'],false,initTable);
                break;
            //自定义头工具栏右侧图标 - 提示
            case 'LAYTABLE_TIPS':
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('真的删除么？', function(index){
                layer.close(index);
                //向服务端发送删除指令
                var resultData = core.ajax('/platform/advertising/delete',false,'POST','id='+data.id);
                if(resultData!=false){
            ...
            core.openIframeDialog('修改','/platform/advertising/input?type=update&id='+data.id,['500px', '900px'],false,initTable);
