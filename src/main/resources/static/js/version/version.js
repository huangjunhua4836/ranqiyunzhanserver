...
layui.use('core', function(){
    ...
    function initTable(where){
        ...
        table.render({
            elem: '#table'
            ,url: '/platform/version/initTable' //数据接口
            ,where: where
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'url', title: '下载路径'}
                ,{field: 'oldversion', title: '上一版本'}
            ]]
            ,page: true //开启分页
        });
    }
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                core.openIframeDialog('添加','/platform/version/input?type=add',['100%', '90%'],false,initTable);
                break;
            //自定义头工具栏右侧图标 - 提示
            case 'LAYTABLE_TIPS':
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            core.openIframeDialog('修改','/crmRole/input?type=update&id='+data.id,['100%', '90%'],false,initTable);
