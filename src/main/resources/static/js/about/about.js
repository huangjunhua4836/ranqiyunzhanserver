...
layui.use('core', function(){
    ...
    function initTable(where){
        ...
        table.render({
            elem: '#table'
            ,url: '/platform/about/initTable' //数据接口
            ,where: where
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                ...
                core.openIframeDialog('添加','/platform/about/input?type=add',['100%', '90%'],false,initTable);
                break;
            //自定义头工具栏右侧图标 - 提示
            case 'LAYTABLE_TIPS':
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            core.openIframeDialog('修改关于','/platform/about/input?type=update&id='+data.id,['100%', '90%'],false,initTable);
