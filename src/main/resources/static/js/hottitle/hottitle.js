...
layui.use('core', function(){
    ...
    function initTable(where){
        ...
        table.render({
            elem: '#table'
            ,url: '/platform/hottitle/initTable' //数据接口
            ,where: where
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                core.openIframeDialog('添加热搜词','/platform/hottitle/input?type=add',['500px', '200px'],false,initTable);
                break;
            //自定义头工具栏右侧图标 - 提示
            case 'LAYTABLE_TIPS':
    ...
    table.on('tool(tableFilter)', function(obj){
        var data = obj.data;
        if(obj.event === 'del'){
            layer.confirm('真的删除热搜词么？绑定该热搜词的用户丢失热搜词！', function(index){
                layer.close(index);
                //向服务端发送删除指令
                var resultData = core.ajax('/platform/hottitle/delete',false,'POST','id='+data.id);
                if(resultData!=false){
            ...
            core.openIframeDialog('修改热搜词','/platform/hottitle/input?type=update&id='+data.id,['500px', '200px'],false,initTable);
        } else if(obj.event === 'detail'){
            core.openDialog('热搜词详情',$('#detail').html(),['500px','480px']);
