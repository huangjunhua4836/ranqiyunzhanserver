...
layui.use('core', function(){
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                core.openIframeDialog('添加','/platform/problem/input?type=add',['100%', '100%'],false,initTable);
                break;
            //自定义头工具栏右侧图标 - 提示
            case 'LAYTABLE_TIPS':
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            core.openIframeDialog('修改','/platform/problem/input?type=update&id='+data.id,['100%', '100%'],false,initTable);
