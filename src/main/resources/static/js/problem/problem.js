...
layui.use('core', function(){
    ...
    function initTable(where){
        ...
        table.render({
            elem: '#table'
            ,url: '/platform/problem/initTable' //数据接口
            ,where: where
    ...
    table.on('toolbar(tableFilter)', function(obj){
        switch(obj.event){
            case 'add':
                core.openIframeDialog('添加','/platform/problem/input?type=add',['100%', '90%'],false,initTable);
                break;
            case 'export':
                location.href = "/platform/problem/export?str="+encodeURIComponent(JSON.stringify(exportcondit));
    ...
    table.on('tool(tableFilter)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('真的删除么？', function(index){
                layer.close(index);
                //向服务端发送删除指令
                var resultData = core.ajax('/platform/problem/delete',false,'POST','id='+data.id);
                if(resultData!=false){
                    layer.msg('操作成功！',core.showtime);
                    initTable({});
                }
            ...
            core.openIframeDialog('修改','/platform/problem/input?type=update&id='+data.id,['100%', '90%'],false,initTable);
