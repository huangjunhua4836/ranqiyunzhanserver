package com.yl.soft.common.jpush;
...
public class JpushUtil {
	 ...
	 * @return 0推送失败，1推送成功
	 */
	public static int sendToBieMing(String bieming, String notification_title, String msg_title, String msg_content,
			Map<String, String> extrasparams) {
		int result = 0;
		try {
			PushPayload pushPayload = JpushUtil.buildPushObject_all_BieMing_alertWithTitle(bieming, notification_title,
					msg_title, msg_content, extrasparams);
			System.out.println(pushPayload);
			PushResult pushResult = jPushClient.sendPush(pushPayload);
			System.out.println(pushResult);
			if (pushResult.getResponseCode() == 200) {
	...
	public static int sendToAllAndroid(String notification_title, String msg_title, String msg_content,
		...
		int result = 0;
		try {
			PushPayload pushPayload = JpushUtil.buildPushObject_android_all_alertWithTitle(notification_title,
					msg_title, msg_content, extrasparams);
			System.out.println(pushPayload);
			PushResult pushResult = jPushClient.sendPush(pushPayload);
			System.out.println(pushResult);
	...
	public static int sendToAllIos(String notification_title, String msg_title, String msg_content,
		...
		int result = 0;
		try {
			PushPayload pushPayload = JpushUtil.buildPushObject_ios_all_alertWithTitle(notification_title, msg_title,
	...
	public static void main(String[] args) {
		sendToAll("标题", "标题", "dfdsfsdfdsf", null);
	}
	 ...
	 * @return 0推送失败，1推送成功
	 */
	public static int sendToAll(String notification_title, String msg_title, String msg_content,
			Map<String, String> extrasparams) {
		int result = 0;
		try {
			PushPayload pushPayload = JpushUtil.buildPushObject_android_and_ios(notification_title, msg_title,
					msg_content, extrasparams);
			System.out.println(pushPayload);
			PushResult pushResult = jPushClient.sendPush(pushPayload);
			System.out.println(pushResult);
			if (pushResult.getResponseCode() == 200) {
		...
		return PushPayload.newBuilder().setPlatform(Platform.android_ios()).setAudience(Audience.all())
				.setNotification(Notification.newBuilder().setAlert(notification_title)
						.addPlatformNotification(AndroidNotification.newBuilder()// android推送设置
								.setAlert(notification_title).setTitle(notification_title)// 设置通知标题
						...
						.addPlatformNotification(IosNotification.newBuilder()// ios推送设置
								...
								.setSound("default").addExtras(extrasparams)
				...
				.setMessage(Message.newBuilder().setMsgContent(msg_content)// 通知内容
	...
	private static PushPayload buildPushObject_all_registrationId_alertWithTitle(String registrationId,
			String notification_title, String msg_title, String msg_content, Map<String, String> extrasparams) {
		return PushPayload.newBuilder()
				...
				.setNotification(Notification.newBuilder()
						...
						.addPlatformNotification(AndroidNotification.newBuilder().setAlert(notification_title)
								.setTitle(notification_title).addExtras(extrasparams).build())
						// 指定当前推送的iOS通知
						.addPlatformNotification(IosNotification.newBuilder()
								// 传一个IosAlert对象，指定apns title、title、subtitle等
								.setAlert(notification_title).incrBadge(1)
								// 此字段的值default表示系统默认声音；传sound.caf表示此推送以项目里面打包的sound.caf声音来提醒，
								// 如果系统没有此音频则以系统默认声音提醒；此字段如果传空字符串，iOS9及以上的系统是无声音提醒，以下的系统是默认声音
								.setSound("sound.caf")
				...
				// sdk默认不做任何处理，不会有通知提示。建议看文档http://docs.jpush.io/guideline/faq/的
				// [通知与自定义消息有什么区别？]了解通知和自定义消息的区别
				.setMessage(Message.newBuilder().setMsgContent(msg_content).setTitle(msg_title).addExtras(extrasparams)
						.build())
	 ...
	 * @return
	 */
	private static PushPayload buildPushObject_all_BieMing_alertWithTitle(String bieming, String notification_title,
			String msg_title, String msg_content, Map<String, String> extrasparams) {
